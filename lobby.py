...
def iniciar_lobby(contexto):
    ...
    # 🔊 Cargar y reproducir música del lobby
    try:
        pygame.mixer.music.load("assets/soundtrack.mp3")
        pygame.mixer.music.play(-1)  # Bucle infinito
        print("🎵 Música del lobby iniciada")
    except pygame.error as e:
        print(f"❌ No se pudo cargar soundtrack.mp3: {e}")
            ...
            if estado_juego["modo"] == "lobby" and not game_over:
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        # 🔊 Detener música del lobby antes de volver al menú
                        pygame.mixer.music.stop()
                        print("⏹️ Música del lobby detenida al volver al menú")
...
def mostrar_pantalla_final(contexto, tipo, porcentaje, tiempo_str, por_denuncias=False):
    ...
    img_path = "assets/completado.png" if tipo == 'completado' else "assets/gameover.png"
    try:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if rect_menu.collidepoint(mouse_pos):
                    ...
                    sys.exit()
        screen.blit(fondo_img, (0, 0))
        screen.blit(txt_result, (ANCHO // 2 - txt_result.get_width() // 2, ALTO // 2 - txt_result.get_height()))
        screen.blit(txt_tiempo, (ANCHO // 2 - txt_tiempo.get_width() // 2, ALTO // 2 + 10))
        if por_denuncias:
            mensaje1 = "Has perdido, recibiste dos denuncias"
            mensaje2 = "por discriminación."
